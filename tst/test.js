...
// len = 5, some identical values, non compressed
var hd = new Rec(5)
hd.push(1)
hd.push(2)
hd.push(3)
...
// len = 5, compressed, sorted (all new max)
var rec = new Rec(5)
testSet(rec, [0,1,2,3,4,5,6,7,8])

// len = 5, reverse sorted (all new min)
rec = new Rec(5)
...
// len = 9, 101 reversed
rnd = []
for (i=0; i<101; ++i) rnd.push(101-i-1)
testSet(new Rec(9), rnd)

// uniform random, m=9, n=1000
n = 1001
rnd = []
rec = new Rec(9)
for (i=0; i<n; ++i) rnd.push(Math.random())
testSet(rec, rnd)

// skewed random, m=9, n=1000
rnd = []
for (i=0; i<1001; ++i) rnd.push(Math.random()+Math.random()*Math.random())
testSet(new Rec(9), rnd)
...
// lots of repeated values n=9, n=1000
rnd = []
for (i=0; i<1000; ++i) rnd.push(Math.random() < 0.2 ? 0 : Math.random()*Math.random())
testSet(new Rec(9), rnd)
...
t('<', rms[3]/n, 1e-12, 'ave'+rnd.reduce(sum)/n+' '+hd.E)
...
function testSet(cdf, set) {
	...
	t('<', Math.abs(cdf.V-1), 1e-1, 'V ~= 1')
	t('<', Math.abs(cdf.S-1), 5e-2, 'S ~= 1')
}
